...
const GameBoardPage = () => {
    ...
    const { gameState, setGameState, setRoom, myPlayer, setMyPlayer, rollDice, sendMove, connect } = useGame();

    const [displayTime, setDisplayTime] = useState(60);
    const [extraRot, setExtraRot] = useState({ x: 0, y: 0 });
    const lastRollIdRef = useRef(0);

    useEffect(() => {
        if (gameState?.rollId && gameState.rollId !== lastRollIdRef.current) {
            lastRollIdRef.current = gameState.rollId;
            const t = setTimeout(() => {
                setExtraRot(prev => ({
                    x: prev.x + 720,
                    y: prev.y + 1080
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE_URL}/api/rooms/${roomCode}`)
            ...
            .then(data => {
                ...
                const savedPlayer = JSON.parse(sessionStorage.getItem('ludo_player'));
                if (savedPlayer) {
                    const roomPlayer = data.players.find(p => p.name.toLowerCase() === savedPlayer.name.toLowerCase());
                    if (roomPlayer) {
    ...
    playableColors.forEach(color => {
        const playerState = gameState.players[color];
        if (!playerState || !playerState.tokens) return;
        
        playerState.tokens.forEach(token => {
    ...
    const getDiceTransform = () => {
        const side = gameState?.lastDiceValue || 1;
        const baseRotations = {
            1: { x: 0, y: 0 },
            2: { x: 0, y: 180 },
            3: { x: 0, y: -90 },
        ...
        const rot = baseRotations[side] || { x: 0, y: 0 };
        return {
            transform: `rotateX(${rot.x + extraRot.x}deg) rotateY(${rot.y + extraRot.y}deg)`,
        ...
        <div className="container ludo-container">
            <section className="game-panel">
                <div className="turn-indicator-box">
                    ...
                    <RankingBoard gameState={gameState} />
                </div>
                ...
                <div className="dice-section">
                    ...
                    <div
                        ...
                        onClick={() => {
                            if (myPlayer?.id && isMyTurn && !gameState.diceRolled && gameState.status !== 'FINISHED') {
                                rollDice(myPlayer.id);
                            }
                        }}
                        role="button"
                        tabIndex={isMyTurn && !gameState.diceRolled ? 0 : -1}
                    >
                        <div className="dice-cube" style={getDiceTransform()}>
            ...
            <div className="ludo-layout-wrapper">
                {playableColors.map(color => (
                    <PlayerCard
                        ...
                        color={color}
                        rank={getPlayerRank(gameState, color)}
                        ...
                        gameState.players[color].tokens.map(token => (
                            <Token
                                ...
                                isClickable={
                                    ...
                                    isTokenMoveLegal(gameState, color, token, gameState.diceValue)
                                }
...
const RankingBoard = ({ gameState }) => {
    const rankings = gameState.rankings || [];
    if (!rankings.length) return null;

    return (
        <div className="ranking-board" aria-label="Player rankings">
            {rankings.slice(0, 3).map((color, index) => {
                const rank = index + 1;
                return (
                    <div key={color} className={`ranking-item rank-${rank}`}>
                        <span className="ranking-place">{RANK_LABELS[rank]}</span>
                        <span className="ranking-name" style={{ '--rank-color': COLORS[color] }}>
                            {gameState.players?.[color]?.name || color}
                        </span>
                    </div>
                );
            })}
...
const PlayerCard = ({ player, color, rank, isCurrentTurn, isMyPlayer }) => (
    <div className={`player-card player-card-${color.toLowerCase()} ${isCurrentTurn ? 'active-turn' : ''} ${isMyPlayer ? 'is-me' : ''}`}>
        <div className="player-avatar" style={{ '--player-color': COLORS[color] }}>
            ...
            {rank > 0 && <span className={`rank-badge rank-${rank}`}>{RANK_LABELS[rank] || `${rank}th`}</span>}
        </div>
        <div className="player-info">
            <span className="player-name">{player?.name || 'Waiting...'}</span>
            <div className="player-meta">
                <span className="player-coins">{"\uD83D\uDCB0"} {player?.coins || 0}</span>
                {rank > 0 && <span className="finish-label">{RANK_LABELS[rank] || `${rank}th`} PLACE</span>}
...
const getAbsoluteTrackIndex = (color, relativePosition) => (
    ((COLOR_OFFSETS[color] || 0) + relativePosition) % MASTER_PATH.length
);

const getPlayerRank = (gameState, color) => {
    const savedRank = gameState.players?.[color]?.rank || 0;
    if (savedRank > 0) return savedRank;

    const rankingIndex = (gameState.rankings || []).indexOf(color);
    return rankingIndex >= 0 ? rankingIndex + 1 : 0;
};

const hasOpponentBlockAt = (gameState, movingColor, absoluteTrackIndex) => (
    Object.entries(gameState.players || {}).some(([color, player]) => {
        if (color === movingColor) return false;

        const tokensOnCell = (player.tokens || []).filter(token => (
            token.position >= 0 &&
            token.position < 52 &&
            getAbsoluteTrackIndex(color, token.position) === absoluteTrackIndex
        ));
...
const isTokenMoveLegal = (gameState, color, token, dice) => {
    if (!token || !dice) return false;

    if (token.position === -1) {
        return dice === 6 && !hasOpponentBlockAt(gameState, color, getAbsoluteTrackIndex(color, 0));
    }

    if (token.position === 57) return false;

    const targetPosition = token.position + dice;
    if (targetPosition > 57) return false;

    if (token.position >= 52) return targetPosition <= 57;
    if (targetPosition >= 52) return true;

    return !hasOpponentBlockAt(gameState, color, getAbsoluteTrackIndex(color, targetPosition));
};
