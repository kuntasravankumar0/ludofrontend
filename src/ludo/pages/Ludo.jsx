...
const handleCreate = async (name, maxPlayers, gameMode, setRoom, setMyPlayer, navigate) => {
    ...
    if (!trimmedName) return alert('Enter name');
    try {
        const res = await fetch(`${API_BASE_URL}/api/rooms/create`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ hostName: trimmedName, maxPlayers: parseInt(maxPlayers), gameMode })
        });
        if (!res.ok) {
...
const handleJoin = async (name, roomCode, setRoom, setMyPlayer, navigate) => {
    ...
    if (!trimmedName || !trimmedCode) return alert('Enter name/code');
    
    try {
        const res = await fetch(`${API_BASE_URL}/api/rooms/join`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ roomCode: trimmedCode, playerName: trimmedName })
        });
        if (!res.ok) {
            const error = await res.text();
            return alert(error);
        ...
        const player = data.players.find(p => p.name.toLowerCase() === trimmedName.toLowerCase());
        if (player) {
            setMyPlayer(player);
        } else {
            console.error('Player not found in room after joining');
        }
